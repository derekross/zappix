...
export const HashtagFeedPage: React.FC = () => {
    ...
    const fetchHashtagFeed = useCallback(async (until?: number) => {
        if (processingRef.current || !ndk || !cleanHashtag || isLoading || isReachingEnd) {
            return;
        }
        processingRef.current = true;
        console.log(`Fetching feed for hashtag: #${cleanHashtag} (Kinds: ${FEED_KINDS.join(',')}), until: ${until ? new Date(until * 1000).toISOString() : 'Now'}`);
        setIsLoading(true);
