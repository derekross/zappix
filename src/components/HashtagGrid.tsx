      ...
      <CardContent className="p-0">
        <div className="grid grid-cols-3 gap-1 lg:gap-2">
          {Array.from({ length: 3 }).map((_, i) => (
            <Skeleton key={i} className="aspect-square rounded-sm lg:rounded-md" />
          ))}
        </div>
      </CardContent>
    </Card>
...
export function HashtagGrid({ onHashtagClick }: HashtagGridProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {hashtagPosts.data?.map(({ hashtag, posts }, index) => {
        ...
        const images = posts.slice(0, 3).map(post => {
          const imetaTag = post.tags.find(([name]) => name === 'imeta');
          if (!imetaTag) return null;
          
          const urlPart = imetaTag.find(part => part.startsWith('url '));
          const url = urlPart?.replace('url ', '');
          const title = post.tags.find(([name]) => name === 'title')?.[1] || '';
