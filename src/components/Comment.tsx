...
export const Comment: React.FC<CommentProps> = ({ event, onReply }) => {
    ...
    useEffect(() => {
        if (event.author) {
            event.author.fetchProfile({closeOnEose: true})
                .then(setAuthorProfile)
                .catch(err => console.error("Failed profile fetch", err));
        }
    }, [event.author]);

    const authorDisplayName = authorProfile?.displayName || authorProfile?.name || event.pubkey.substring(0, 12);
            ...
            {/* Content Section */}
             <Box sx={{ flexGrow: 1 }}>
                 ...
                 <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
                     <RouterLink to={`/profile/${event.author.npub}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  ...
                  {/* Comment Text */}
                 <Typography variant="body2" sx={{ my: 0.5, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}> {/* Allow line breaks */}
                     {event.content}
                 </Typography>
                  {/* Reply Button */}
                 {signer && (
