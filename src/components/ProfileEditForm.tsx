...
export const ProfileEditForm: React.FC<ProfileEditFormProps> = ({
    ...
    useEffect(() => {
        if (open) { // Only check when modal becomes visible
    ...
    const handleFileChange = async (event: ChangeEvent<HTMLInputElement>, fieldName: 'image' | 'banner') => {
        ...
        try {
            ...
            console.log(`Uploading ${file.name} (${Math.round(file.size / 1024)} KB) via PUT to ${uploadEndpoint}...`); 
            ...
            toast.error(`Upload failed: ${error instanceof Error ? error.message : String(error)}`, { id: uploadToastId });
        } finally {
            setLoading(false);
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent>
                <Stack spacing={2} sx={{ mt: 1 }}>
                    {/* Standard Fields */}
                    <TextField label="Display Name" name="displayName" value={profile.displayName || ''} onChange={handleChange} fullWidth disabled={isUploading}/>
                    <TextField label="Username (handle)" name="name" value={profile.name || ''} onChange={handleChange} fullWidth disabled={isUploading}/>
                    <TextField label="About" name="about" value={profile.about || ''} onChange={handleChange} fullWidth multiline rows={3} disabled={isUploading}/>
                    ...
                    {/* Other Fields */}
                    <TextField label="Website URL" name="website" value={profile.website || ''} onChange={handleChange} fullWidth type="url" disabled={isUploading}/>
                    <TextField label="Nostr Address (NIP-05)" name="nip05" value={profile.nip05 || ''} onChange={handleChange} fullWidth disabled={isUploading}/>
                    <TextField label="Lightning Address (LUD-16)" name="lud16" value={profile.lud16 || ''} onChange={handleChange} fullWidth disabled={isUploading}/>
                </Stack>
            </DialogContent>
            <DialogActions>
                <Button onClick={onClose} disabled={isSaving || isUploading}>Cancel</Button>
