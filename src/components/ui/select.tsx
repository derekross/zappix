...
export type SelectProps = SelectRootProps & {
  ...
  options:
    | Record<string, Array<{ label: React.ReactNode; item: Omit<SelectItemProps, "children"> }>>
    | Array<{ label: React.ReactNode; item: Omit<SelectItemProps, "children"> }>;
  selectTriggerProps?: SelectTriggerProps;
};

export const Select: React.FC<SelectProps> = (props) => {
  const { placeholder, options, selectTriggerProps = {}, ...selectRootProps } = props;
    ...
    <SelectRoot {...selectRootProps}>
      ...
      <SelectContent>
        {Array.isArray(options) &&
          options.map((option) => {
            return (
              <SelectItem {...option.item} key={JSON.stringify(option)}>
                {option.label}
              </SelectItem>
            );
          })}
        {!Array.isArray(options) &&
          Object.entries(options).map(([groupLabel, groupOptions]) => {
            return (
              <SelectGroup key={JSON.stringify(groupOptions)}>
