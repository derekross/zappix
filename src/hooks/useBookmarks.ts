...
const IMAGE_BOOKMARKS_LIST = "NIP-68-Posts";
...
export function useBookmarks() {
  ...
  return useQuery({
    ...
    queryFn: async (c) => {
      ...
      // Get user's bookmark list (kind 10003) with the specific list name
      const bookmarkEvents = await nostr.query(
        [
          {
            kinds: [10003],
            authors: [user.pubkey],
            "#d": [IMAGE_BOOKMARKS_LIST],
            limit: 1,
          },
        ],
        { signal }
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (bookmarkEvents.length > 0) {
        ...
        // If no bookmark list exists, create one with the list name
        currentTags = [["d", IMAGE_BOOKMARKS_LIST]];
      }
